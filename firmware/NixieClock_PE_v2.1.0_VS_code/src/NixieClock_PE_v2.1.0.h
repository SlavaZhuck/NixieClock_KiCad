...
// Автоподстройка времени для компенсации отклонения кварца.
// Сдвиг применяется один раз при смене календарного месяца (по данным RTC).
// Значение задаётся в секундах и может быть отрицательным
// (отрицательное - отвести часы назад, положительное - вперёд).
...
enum FLIP_MODES : byte
{
  ...
  FM_ELASTIC
}; // (5) резинка (рекомендуемая скорость: 50-150), срабатывает только на минутах
...
enum SH_MODES : byte
{
  ...
  SHATM
}; // (6) отображение атмосферного давления (5 сек)
...
enum SAVE_PARAMS : byte
{
  ...
  LASTADJMONTH  // (9) последний месяц, в котором была применена коррекция ADJUST_TIME
};
