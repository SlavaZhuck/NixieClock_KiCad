  ...
  switch (curMode) {
    ...
    case SHTIME:                                  // (0) отображение часов
      ...
      if (btnA.isClick() || btnA.isHolded()) {    // переход в режим отображения температуры
        curMode = SHTEMP;
        opts[4] = DOT;
        indiDigits[4] = IN_CELS;
        if (isBMEhere) { 
          bme_temp->getEvent(&temp_event);
          bme_pressure->getEvent(&pressure_event);
          bme_humidity->getEvent(&humidity_event);
          indiDigits[2] = (byte)((int)temp_event.temperature / 10);
          indiDigits[3] = (byte)((int)temp_event.temperature % 10);
          anodeStates = 0x1C; 
        } else anodeStates = 0x10;
      ...
      if (measurementsTimer.isReady()) {
        if (isBMEhere) {
          ...
          if ((int)humidity_event.relative_humidity == 100) {
            ...
            indiDigits[1] = (byte)((int)humidity_event.relative_humidity % 10);
            anodeStates = 0x1C;
          }
        }
      }
